"""
===============================================================================
Script complexity Index — Font Metric Extraction Pipeline
===============================================================================
...
Purpose:  Extract Expansion Ratio (E), Vertical Footprint (V), and
          ...
          complexity Index that quantifies how much "technical debt"
...
Usage:
    ...
    2. Run this script:     python script_complexity_pipeline.py
    3. Output:              complexity_index_results.csv
===============================================================================
"""
...
TARGET_SCRIPTS = {
    "Latin":      [(0x0000, 0x007F), (0x0080, 0x00FF), (0x0100, 0x017F)],  # Basic Latin + Latin-1 Supplement + Latin Extended-A
...
def compute_complexity_index(
    ...
    and invert to get the complexity Index.
...
def print_report(result_df: pd.DataFrame):
    ...
    print("\n" + "=" * 80)
    print("SCRIPT complexity INDEX — RESULTS")
    print("=" * 80)
    ...
    print("\n\n📐 Normalized Scores & complexity Index:\n")
    norm_cols = [
        "script", "E_norm", "V_norm", "F_norm",
        "complexity_C"
    ]
    print(result_df[norm_cols].sort_values("complexity_C").to_string(
    ...
    most_complex = result_df.loc[result_df["complexity_C"].idxmin()]
    most_similar = result_df.loc[result_df["complexity_C"].idxmax()]
    print(f"\n  Most complex script:    {most_complex['script']} (S = {most_complex['complexity_C']:.3f})")
    print(f"  Most Latin-like script: {most_similar['script']} (S = {most_similar['complexity_C']:.3f})")
...
def run_complexity_pipeline(force: bool = False) -> Path:
    ...
    Full pipeline execution:
      ...
      3. Normalize and compute complexity Index
      4. Export results
    """
    ...
    COMPLEXITY_DATA_DIR.mkdir(parents=True, exist_ok=True)
    ...
    raw_df.to_csv(COMPLEXITY_DATA_DIR / "raw_font_metrics.csv", index=False)
    logger.info("Saved raw metrics → complexity/raw_font_metrics.csv")

    # --- Step 3: Aggregate per script ---
    logger.info("Step 3: Aggregating per script (median)...")
    agg_df = aggregate_per_script(raw_df)

    # --- Step 4: Normalize and compute complexity Index ---
    logger.info("Step 4: Computing complexity Index...")
    result_df = compute_complexity_index(agg_df)

    # --- Step 5: Report and export ---
    print_report(result_df)

    result_df.to_csv(COMPLEXITY_DATA_DIR / "complexity_index_results.csv", index=False)
    logger.info("Saved results → complexity/complexity_index_results.csv")

    # Also save a clean summary for merging with Exposure + Support
    summary = result_df[["script", "complexity_C",
                          "median_expansion_ratio", "median_vertical_footprint",
                          "median_friction_score", "font_count"]].copy()
    summary.to_csv(COMPLEXITY_DATA_DIR / "complexity_index_summary.csv", index=False)
    logger.info("Saved summary → complexity/complexity_index_summary.csv")

    return COMPLEXITY_DATA_DIR

def main() -> None:
    run_complexity_pipeline(force=True)
