"""
===============================================================================
Script Diversity Index — CNN-B/16 Embedding Pipeline (100-Glyph Edition)
===============================================================================
...
Changes from 10-glyph version:
    ...
    - Better signal for the CNN to distinguish font styles
    ...
    CNN a comprehensive view of each font's design decisions.

Architecture: CNN-B/16 (Vision Transformer, Base, 16×16 patches)
...
Usage:
    ...
    2. Run: python script_diversity_cnn_100.py
    3. Output: cnn_outputs_100/diversity_index_results.csv
===============================================================================
"""
...
OUTPUT_DIR = Path("./cnn_outputs_100")

FONT_SIMILARITY_DIR = Path("./cnn_outputs_100/font_similarity_pairs")
...
class ResNetFeatureExtractor:
    """
    Pretrained ResNet-50 as a frozen feature extractor.
    Input: 224×224 grayscale glyph image → Output: 2048-dim L2-normalized embedding.
    """
    ...
    def __init__(self, device: str = "cpu"):
        ...
        # Remove the classification head — output is now the 2048-dim avg pool features
        ...
        logger.info("ResNet-50 ready (2048-dim embeddings)")
    ...
    def extract_batch(self, images: List[Image.Image], batch_size: int = 64) -> np.ndarray:
        ...
        for i in range(0, len(images), batch_size):
            ...
            emb = emb.squeeze(-1).squeeze(-1).cpu().numpy()  # flatten (N, 2048, 1, 1) → (N, 2048)
...
def run_pipeline(fonts_dir: Path) -> pd.DataFrame:
    ...
    extractor = ResNetFeatureExtractor(device="cuda" if torch.cuda.is_available() else "cpu")
    print("Using GPU for feature extraction") if torch.cuda.is_available() else print("Using CPU for feature extraction")
...
def print_report(df: pd.DataFrame):
    print("\n" + "=" * 80)
    print("SCRIPT DIVERSITY INDEX — ResNet 50 (100-glyph edition)")
    print("=" * 80)
...
def main():
    ...
    logger.info("Starting CNN Diversity Pipeline (100-glyph edition)...")
